<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0f2027,100:2c5364&height=180&section=header&text=Kenji%20Mattos%20Kinoshita&fontSize=38&fontColor=ffffff&animation=fadeIn&fontAlignY=35&desc=Software%20Engineer%20%7C%20Frontend%20%26%20Product-Focused&descSize=16&descAlignY=55"/>

<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=20&duration=3000&pause=1000&color=2C5364&center=true&vCenter=true&repeat=true&width=650&height=45&lines=Building+product-first+web+experiences;Frontend+engineering+with+real+business+impact;Design-to-code+execution+and+scalable+interfaces)](https://git.io/typing-svg)

<br/>

<a href="https://www.linkedin.com/in/kenjimattos/">
  <img src="https://img.shields.io/badge/-LinkedIn-203a43?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>
<a href="mailto:kenjimattos@gmail.com">
  <img src="https://img.shields.io/badge/-Email-2c5364?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>
<a href="https://kenji-mattos.vercel.app">
  <img src="https://img.shields.io/badge/-Portfolio-0f2027?style=for-the-badge&logo=vercel&logoColor=white"/>
</a>

<br/><br/>

<img src="https://komarev.com/ghpvc/?username=kenjimattos&style=for-the-badge&color=203a43" alt="Profile Views"/>

</div>

---

## :technologist: About Me

```typescript
const kenji = {
  role: "Software Engineer | Frontend & Product-Focused",
  location: "Sao Bernardo do Campo, Sao Paulo, Brazil",

  bio: `Software Engineer with strong experience in frontend development
        using React and TypeScript for scalable, client-facing products.
        Product-minded, performance-focused, and experienced in cross-team
        collaboration across design, engineering, and stakeholders.`,

  journey: {
    start: "Design and motion for digital products",
    evolution: "Frontend engineering and product design-to-code",
    current: "Full stack collaboration with a frontend focus"
  },

  stack: {
    languages: ["TypeScript", "JavaScript"],
    frontend: ["React", "Next.js", "HTML5", "CSS", "Tailwind CSS"],
    backend: ["Node.js", "REST APIs", "Auth and business rules"],
    databases: ["PostgreSQL", "Supabase"],
    product: ["Figma", "UX flows", "Interface architecture"],
    devops: ["Git", "GitHub", "CI/CD basics"]
  },

  expertise: [
    "Product-focused frontend development",
    "Design-to-code execution with UX clarity",
    "API integration and scalable interfaces",
    "Cross-functional collaboration in agile teams"
  ],

  philosophy: "Build the right thing, and build it well."
};
```

<br/>

## :zap: Tech Stack

<img src="https://skillicons.dev/icons?i=react,nextjs,ts,js,nodejs,postgres,tailwind,figma,github&perline=6&theme=light" />

</div>

<br/>

## :rocket: Professional Experience

### <img src="https://img.shields.io/badge/Revoluna-203a43?style=flat-square&logoColor=white" height="20"/> Full Stack Developer (Frontend-Focused)

**Jan 2025 - Present | Sao Paulo, Brazil (Hybrid)**

- Scaled a production web platform with React, Next.js, and TypeScript
- Built interactive interfaces aligned with product and business goals
- Designed and integrated REST APIs across frontend and backend
- Delivered RBAC and backend logic with Node.js + SQL databases
- Led a database refactor for clarity and long-term maintainability

</td>
<td width="50%">

### <img src="https://img.shields.io/badge/Revoluna-2c5364?style=flat-square&logoColor=white" height="20"/> Product Designer

**Oct 2024 - Jan 2025 | Sao Paulo, Brazil**

- Redesigned a mobile product and defined UX flows
- Launched a functional MVP using low-code + custom code
- Bridged product, design, and engineering decisions

### <img src="https://img.shields.io/badge/Tactical-0f2027?style=flat-square&logoColor=white" height="20"/> Motion Designer

**Apr 2021 - Oct 2023 | Remote - Dubai, UAE**

- Shipped digital products and interactive experiences for global clients
- Delivered in fast-paced, fully remote teams

### <img src="https://img.shields.io/badge/Freelance-203a43?style=flat-square&logoColor=white" height="20"/> Motion Designer

**Mar 2017 - Apr 2021 | Remote - Brazil**

- Led projects end-to-end and managed direct client communication
- Built strong delivery discipline across multidisciplinary teams

<br/>

## :mortar_board: Education

- **Bachelor's Degree in Economics** — Universidade Presbiteriana Mackenzie (Sao Paulo, Brazil)
- **Associate Degree in Animation** — Faculdade Melies (Sao Paulo, Brazil)

<br/>

<div align="center">

[![LinkedIn](https://img.shields.io/badge/Connect_on_LinkedIn-203a43?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/kenjimattos/)

[![CV Download](https://img.shields.io/badge/CV%20Download-203a43)](Kenji%20Mattos%20Kinoshita.pdf)

</div>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0f2027,50:203a43,100:2c5364&height=120&section=footer"/>
